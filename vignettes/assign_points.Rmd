...
## Get some data
This example uses commercial landings data, that have the same longitude and latitude coordinates with the same crs as the shapefile.
```{r results = "hold"}
...
Finally, the previous examples show that a number of data points don't overlap with any of the polygons in our shapefiles. But what if we wanted to find out which polygons they were closest to? Then we could use function `assign_points_to_nearest_polygon`. This function takes a bit longer to run compare to `assign_points_terra` or `assign_points_secr`.
